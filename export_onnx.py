...
class StaticGaussianBlur(torch.nn.Module):
    ...
    def __init__(self, kernel_size: int, sigma: float):
        ...
        # Reshape for depthwise conv2d (anomaly map has 1 channel)
        kernel_2d = kernel_2d.view(1, 1, kernel_size, kernel_size)
        
        # Register as a buffer so it becomes a static constant in ONNX
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("weights_path", type=str, help="Path to the trained weights file")
...
def main():
    ...
    # Patch Dynamic GaussianBlur (CORRECTED PATH)
    if hasattr(model, 'anomaly_map_generator') and hasattr(model.anomaly_map_generator, 'blur'):
        ...
        if hasattr(blur_module, 'kernel_size') and hasattr(blur_module, 'sigma'):
            ...
            print(f"Patching anomaly_map_generator.blur -> StaticGaussianBlur (kernel={k_size}, sigma={sigma})")
            model.anomaly_map_generator.blur = StaticGaussianBlur(kernel_size=k_size, sigma=sigma).to(device)
        else:
            print("Warning: Target blur module lacks kernel_size/sigma. Bypassing internal blur.")
            model.anomaly_map_generator.blur = torch.nn.Identity()
    else:
        print("Warning: anomaly_map_generator.blur not found. Attempting standard export.")
    ...
    # Export to ONNX
    print(f"Compiling computational graph to {onnx_path}...")
    torch.onnx.export(
        model,
        dummy_input,
        str(onnx_path),
